...
class Cube {
  ...
  twist(item, {corners = true, edges = true, centers = true} = {}) {
    if (item instanceof Algorithm) {
      ...
      if (item.inverseTwists.length || item.inverseRotation) {
        this.inverse()
        for (let twist of item.inverseTwists) {
          this.twist(twistCubes[twist], {corners, edges, centers})
        }
      ...
      if (this === item) {
        if (corners) {
          let new_corners = new Array(8)
          for (let i = 0; i < 8; ++i) {
            let transform = this.corners[Math.floor(this.corners[i] / 3)]
            new_corners[i] = transform - transform % 3 + (this.corners[i] + transform) % 3
          }
          this.corners = new_corners
        }
        if (edges) {
          let new_edges = new Array(12)
          for (let i = 0; i < 12; ++i) {
            new_edges[i] = this.edges[this.edges[i] >>> 1] ^ this.edges[i] & 1
          }
  ...
  inverse() {
    let corners = new Array(8)
    for (let i = 0; i < 8; ++i) {
      let item = this.corners[i]
  ...
  toFaceletString() {
    ...
    let list = new Array(54)
    for (let i = 0; i < 8; ++i) {
      let permutation = Math.floor(this.corners[i] / 3)
      let orientation = this.corners[i] % 3
      for (let j = 0; j < 3; ++j) {
        let position = cornerFacelets[permutation][(orientation + j) % 3]
        list[offset[position[0]] * 9 + Number.parseInt(position[1])] = cornerFacelets[i][j][0]
      }
    }
    for (let i = 0; i < 12; ++i) {
      let permutation = this.edges[i] >>> 1
      let orientation = this.edges[i] & 1
      for (let j = 0; j < 2; ++j) {
        let position = edgeFacelets[permutation][orientation ^ j]
        list[offset[position[0]] * 9 + Number.parseInt(position[1])] = edgeFacelets[i][j][0]
      }
    }
    let table = rotationPermutationTable[inverseCenterTable[this.placement]]
    for (let i = 0; i < 6; ++i) {
