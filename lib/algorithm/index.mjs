...
import {rotationPermutationTable, inverseCenterTable} from '../cube/center'
...
export default class Algorithm {
  ...
  clearFlags(placement = 0) {
    let permutation = rotationPermutationTable[inverseCenterTable[this.rotation]]
    let placementPermutation = rotationPermutationTable[placement]
    let transform = [permutation[0], permutation[2], permutation[4]]
    let placementTransform = [placementPermutation[0], placementPermutation[2], placementPermutation[4]]
    this.twists.push(...this.inverseTwists.map(
      twist => transformTwist(transform, transformTwist(placementTransform, inverseTwistTable[twist]))
    ).reverse())
    this.rotation = 0
    this.inverseTwists = []
