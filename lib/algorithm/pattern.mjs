...
export function transformTwist(transform, twist) {
...
export function parseAlgorithmString(string) {
  let twists, inverseTwists, inversed
  if (string.includes('(')) {
    let result = parseBrackets(string)
    if (!result) {
      return null
    }
...
function parseBrackets(string) {
  ...
  while (string) {
    ...
    if (begin === -1) {
      let parseResult = parseString(string)
      if (!parseResult) {
        return null
      }
      twists.push(...parseResult)
